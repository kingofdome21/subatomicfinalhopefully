...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateLanguage(lang) {
        ...
        // IMPORTANT: Update ALL particle and quark info headings regardless of visibility
        ...
        allStrongs.forEach(strong => {
            if (strong.textContent.includes('Mass') || strong.textContent.includes('Masa') || strong.textContent.includes('Kütle')) {
                strong.textContent = translations[lang].mass;
            } else if (strong.textContent.includes('Charge') || strong.textContent.includes('Naboj') || strong.textContent.includes('Yük')) {
                strong.textContent = translations[lang].charge;
            } else if (strong.textContent.includes('Components') || strong.textContent.includes('Komponente') || strong.textContent.includes('Bileşenler')) {
    ...
    // Function to update visible info panels
    function updateVisibleInfoPanels() {
        // Check which particle info is visible and update its title
        if (electronInfo.style.display === 'block') {
            document.querySelector('#electron-info h3').textContent = translations[currentLang].electron;
        }
        if (nucleusInfo.style.display === 'block') {
            document.querySelector('#nucleus-info h3').textContent = translations[currentLang].nucleus;
        }
        
        // Check which quark info is visible and update its title
        if (upInfo.style.display === 'block') {
            document.querySelector('#up-info h3').textContent = translations[currentLang].upQuark;
        }
        if (downInfo.style.display === 'block') {
            document.querySelector('#down-info h3').textContent = translations[currentLang].downQuark;
        }
        if (strangeInfo.style.display === 'block') {
            document.querySelector('#strange-info h3').textContent = translations[currentLang].strangeQuark;
        }
        if (charmInfo.style.display === 'block') {
            document.querySelector('#charm-info h3').textContent = translations[currentLang].charmQuark;
        }
        if (bottomInfo.style.display === 'block') {
            document.querySelector('#bottom-info h3').textContent = translations[currentLang].bottomQuark;
        }
        if (topInfo.style.display === 'block') {
            document.querySelector('#top-info h3').textContent = translations[currentLang].topQuark;
        }
    }
    ...
    langButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            // Force a complete refresh of all language elements
            updateLanguage(this.dataset.lang);
            
            // Force update of all visible panels
            if (electronInfo.style.display === 'block') {
                document.querySelector('#electron-info h3').textContent = translations[currentLang].electron;
            }
            if (nucleusInfo.style.display === 'block') {
